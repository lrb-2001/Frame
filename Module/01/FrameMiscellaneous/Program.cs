...
//builder.Services.AddSingleton(new AppSettings(builder.Configuration));//添加appsetting.json配置文件服务

builder.WebHost.ConfigureAppConfiguration((hostContext, configurationBuilder) =>
{
    new AppSettings(configurationBuilder, hostContext.HostingEnvironment);// 加载配置,多个json文件
});
