...
public class HomeController : BaseController
{
    ...
    public string CreateQRCode(string content)
    {
        try
        {
            byte[] imageBytes = QRCoderHelper.GetPTQRCode(content, 20);

            #region 文件存入本地
            //string filePath = Path.Combine(Directory.GetCurrentDirectory(), "QRCodes", "qr_code.png");
            //System.IO.File.WriteAllBytes(filePath, imageBytes);
            #endregion

            var fileUrl = FTPHelper.UploadFileToFtp(imageBytes, "QRCode","png");
